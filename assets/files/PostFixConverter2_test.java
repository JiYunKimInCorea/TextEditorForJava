...
import com.gsoft.common.Compiler.CodeChar;
...
public class PostFixConverter2_test {
	...
	/** ArrayList listOfVarUses(구분자이면 null, 변수나 함수호출이면 해당 varUse를 참조한다) 멤버를 갖는다. */
	static class CodeStringEx extends CodeString {
		...
		public CodeStringEx(CodeChar[] text, int len, ArrayListInt indicesInSrc, ArrayList listOfVarUses) {
			super(text, len);
		...
		public CodeStringEx(String str, int textColor, ArrayListInt indicesInSrc, ArrayList listOfVarUses) {
			super(str, textColor);
			this.indicesInSrc = indicesInSrc;
			this.listOfVarUses = listOfVarUses;
		}
	...
	CodeStringEx[] Convert()
	{
		...
		ArrayListCodeString result2 = new ArrayListCodeString(0);
	...
	CodeStringEx[] ConvertInfixToPostfix(CodeStringEx[] tempBuffer) {
		for (int i=0; i<tempBuffer.length && tempBuffer[i]!=null; i++) {			
			if (tempBuffer[i].equals("*") || tempBuffer[i].equals("/")) {
				...
				indices.add(-1);
				ArrayList varUses = new ArrayList(1);
				varUses.add(null);
				// :T연결
