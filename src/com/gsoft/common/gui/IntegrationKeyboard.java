...
/* 		[0]	44032	가
        ...
        // 44031 공백
		// 28의 차이 (종성자음이 27개이므로) 
        /* 44032	가       44033	각      44034      44035 	 44059 갛
		...
		static public void resetBuffer() {
			lenOfBuffer = 0;
		}
		...
		//static public boolean isNextToCursor;
		
		static String[] bufferFor겹받침 = new String[2];
		
		static boolean isBkSpPressed = false;
		
		/** 키보드의 리스너인 editText, editRichText의 커서를 전진시킨다.
		 */
		static public void forwardCursor() {
			//mode = Mode.None;
			//lenOfBuffer = 0;
			if (CommonGUI.keyboard!=null) {
				if (CommonGUI.keyboard.listener!=null) {
					if (CommonGUI.keyboard.listener instanceof EditText) {
						EditText editText = (EditText)CommonGUI.keyboard.listener;
						if (editText.textArray[editText.cursorPos.y].count>=editText.cursorPos.x) {
							editText.cursorPos.x++;
						}
					}
					else if (CommonGUI.keyboard.listener instanceof EditRichText) {
						EditRichText editText = (EditRichText)CommonGUI.keyboard.listener;
						if (editText.textArray[editText.cursorPos.y].count>=editText.cursorPos.x) {
							editText.cursorPos.x++;
						}
					} 
		...
		static String convert(String str) throws Exception {
			...
			isBkSpPressed = false;
			if (str.equals(IntegrationKeyboard.Space)|| str.equals(IntegrationKeyboard.Delete) || 
					str.equals(IntegrationKeyboard.Enter)) {
				if (mode!=Mode.None) {
					//isNextToCursor = true;
					Hangul.forwardCursor();
					mode = Mode.None;
					lenOfBuffer = 0;							
					return str;
				}
			...
			else if (str.equals(IntegrationKeyboard.BackSpace)) {
	...
	public static boolean EditTextOfLangDialogIsTouched;
	...
	public IntegrationKeyboard(View owner, Rectangle bounds, Mode mode, 
			int widthOfGap, int heightOfGap, 
			int colorOfButton, int alpha) {
		super();
		this.bounds = bounds;
		this.applySizingBorderOfView(this.bounds);
		this.mode = mode;
		this.owner = owner;
		...
		createKeyboard(Mode.Math, (float)widthOfGap, (float)heightOfGap, colorOfButton, alpha, listener);
		changeMode(Mode.Eng);
		createKeyboard(Mode.Eng, (float)widthOfGap, (float)heightOfGap, colorOfButton, alpha, listener);
		
		CommonGUI.keyboard = this;
	...
	public void changeBounds(Rectangle bounds) {
		...
		for (k=0; k<2; k++) {
			...
			for (i=0; i<ConstantButtonsCountHorz; i++) {				
				boundsOfButton = new Rectangle(x,y,w,h);
	...
	void createKeyboard(Mode mode, float widthOfGap, float heightOfGap, 
		...
		x = (int) (bounds.x + widthOfGap);
		y = (int) (bounds.y + heightOfGap);
		w = (int) ((bounds.width - (ConstantButtonsCountHorz+1) * widthOfGap) / ConstantButtonsCountHorz); 
		h = (int) ((bounds.height - (buttonCountVert+1) * heightOfGap) / buttonCountVert);
		for (i=0; i<ConstantButtonsCountHorz; i++) {				
			boundsOfButton = new Rectangle(x,y,w,h);
			buttons[count] = new Button(this, buttonTextArray[count], buttonTextArray[count], 
					colorOfButton, boundsOfButton, false, alpha, true, 0.0f, null, paint.getColor());
			// 이벤트를 이 클래스에서 직접 처리
		...
		x = (int) (bounds.x+widthOfGap);
		y = (int) (y + heightOfGap);
		w = (int) ((bounds.width - (buttonCountHorz+1) * widthOfGap) / buttonCountHorz); 
		h = (int) ((bounds.height - (buttonCountVert+1) * heightOfGap) / buttonCountVert);
		...
		if (hides) {
			...
			if (listener instanceof EditRichText) {
				...
				if (editText.owner instanceof FileDialog) {
					...
					CommonGUI.fileDialog.open(this, "FileExplorer");
				}
				else { //if (editText.owner instanceof FileDialog==false) {
					if (editText.owner instanceof EditableDialog == false) {
						if (editText.owner instanceof Terminal) {
							/*Terminal terminal = (Terminal) editText.owner; 
							Rectangle paramBounds = new Rectangle(terminal.bounds.x,terminal.bounds.y,
									terminal.bounds.width,view.getHeight()-terminal.bounds.y);
							if (terminal.getHides()==false) {
								terminal.changeBounds(paramBounds);
								terminal.setHides(false);
							}
							else 
								terminal.setHides(false);*/
							if (editText.getHides()==false) {
								if (editText.getIsSingleLine()==false) {
									if (editText.isMaximized()==false)
										editText.setMaximized(true);
								}
							}
						}
						else { // 일반 EditText 컨트롤
							if (editText.getHides()==false) {
								if (editText.getIsSingleLine()==false) {
									if (editText.isMaximized()==false)
										editText.setMaximized(true);
					...
					else { // if (editText.owner instanceof EditableDialog == true) {
						EditableDialog dialog = (EditableDialog) editText.owner;						
						if (dialog.isMaximized()==false) {
							Rectangle paramBounds = new Rectangle(0,0,view.getWidth(),view.getHeight());
							if (dialog.getHides()==false) {
								dialog.changeBounds(paramBounds);
								dialog.setHides(false);
							}
							else 
								dialog.setHides(false);	
							
						}
					}// if (editText.owner instanceof EditableDialog == true) {
				}//if (editText.owner instanceof FileDialog==false) {
			}//else if (listener instanceof EditText) {
		}//if (hides) {
	}
	...
	public boolean onTouch(MotionEvent event, SizeF scaleFactor) {
    	...
    	// 안드로이드에서는 ActionDoubleClicked를 포함하지 않고
    	// 자바에서는 이 이벤트를 포함한다.
    	boolean includesActionDoubleClicked = false;
    	if (Control.CurrentSystem.equals(Control.CurrentSystemIsAndroid))
    		includesActionDoubleClicked = false;
    	else includesActionDoubleClicked = true;
    	
    	if (event.actionCode==MotionEvent.ActionDown || includesActionDoubleClicked) {
    		int i;
    		boolean r=false;
    		if (mode==Mode.Math) {
	    		for (i=0; i<mathButtons.length; i++) {
	...
	public void onTouchEvent(Object sender, MotionEvent e) {
		...
		if (sender instanceof HardwareKeyboard) {
			...
			if (k==null) return;
			if (k.equals("Shift") || k.equals("Alt") || k.equals("Ctrl")) return;
			
			// 키보드 가속기일 경우 
			// key 는 ctrl 이나 alt 와 함께 눌린 대문자 "Z", "Y", "F"등이 된다.
			...
			processEventInCaseEditTextOrEditRichText(e);
		}
		// 하드웨어 키보드가 아닌 화면 키보드의 버튼이 눌린 경우
	...
	/** Ctrl+C, Ctrl+F 등 readOnly 모드에서 사용가능한 키보드 가속기가 눌렸는지 확인한다.*/
	boolean isKeyboardExcelatorPressedInReadOnlyMode() {
		if (isCtrlPressed || isAltPressed) {
			if (key.equals("X") || key.equals("V")) {
				return false;
			}
			return true;
		}
	...
	/** 키보드의 리스너가 EditText나 EditRichText이면 
		isReadOnly를 확인해서 읽기모드일 경우 이벤트를 보내지 않는다.*/
	void processEventInCaseEditTextOrEditRichText(MotionEvent e) {
		if (listener!=null) {
			if (listener instanceof EditText) {
				EditText editText = (EditText)listener;
				if (editText.isReadOnly)  {
					// Ctrl+C, Ctrl+F 와 같은 키보드 가속기는 읽기 모드에서도 이벤트를 전달한다.
					if (isKeyboardExcelatorPressedInReadOnlyMode()) {
			...
			else if (listener instanceof EditRichText) {
				EditRichText editText = (EditRichText)listener;
				if (editText.isReadOnly) {
					// Ctrl+C, Ctrl+F 와 같은 키보드 가속기는 읽기 모드에서도 이벤트를 전달한다.
					if (isKeyboardExcelatorPressedInReadOnlyMode()) {
						listener.onTouchEvent(this, e);
